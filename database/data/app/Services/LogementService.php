...
class LogementService
{
    ...
    public function updateStatus(int $id, string $statut, int $ownerId, ?int $proprieteId = null): ?Logement
    {
        $query = Logement::whereHas('propriete', fn($q) => $q->where('proprietaire_id', $ownerId));

        // Si un proprieteId est fourni, on vérifie aussi la hiérarchie propriete → logement
        if ($proprieteId !== null) {
            $query->where('propriete_id', $proprieteId);
        }

        $logement = $query->find($id);
    ...
    {
        // Fix N+1 : on charge propriete.commune et photos en une seule passe
        return Logement::where('propriete_id', $proprieteId)
            ->with(['propriete.commune', 'photos'])
            ->get();
    }
    ...
    {
        // Fix Haversine : la formule est calculee une seule fois via la sous-requete.
        // On utilise HAVING sur l'alias "distance" pour filtrer sans re-evaluer
        // les fonctions trigonometriques (acos, cos, sin, radians) 3 fois par ligne.
        $formula = '(6371 * acos(
        ...
        return Logement::join('proprietes', 'logements.propriete_id', '=', 'proprietes.id')
            ...
            ->havingRaw('distance <= ?', [$radius])   // ← HAVING sur l'alias au lieu de recalculer
            ->orderByDesc('logements.is_highlighted')
            ->orderBy('distance')                      // ← ORDER BY alias, pas formule
            ->with(['propriete.commune', 'photos'])
            ->get();
    }
