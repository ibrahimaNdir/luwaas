...
class WebhookService
{
    ...
    private function handleSubscription(Transaction $transaction, string $statut, string $transactionRef): array
    {
        ...
        switch ($statut) {
            case 'completed':
                $currentEndsAt = $subscription->proprietaire->subscription_ends_at;
                $baseDate = ($currentEndsAt instanceof \Carbon\Carbon && $currentEndsAt->isFuture())
                    ? $currentEndsAt->copy()
                    : now();
                ...
                event(new \App\Events\PaiementAbonnementRecu($subscription->fresh(['plan', 'proprietaire.user'])));
