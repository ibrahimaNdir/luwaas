...
class BailProprietaireResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        $createdAt = Carbon::parse($this->created_at);
        $dateLimitePaiement = $createdAt->copy()->addDays(7);
        $joursRestants = max(0, $dateLimitePaiement->diffInDays(Carbon::now()));
        ...
        return [
            ...
            'date_limite_paiement' => $dateLimitePaiement->format('Y-m-d'),
            'jours_restants_paiement' => (int)$joursRestants,
        ];
    }
}
